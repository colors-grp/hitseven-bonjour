...
class Secure extends CI_Controller {
	...
	function index() {
		...
		$result = $this->core_fbutils->get_access_token();
	...
	function me($object = NULL) {
		if ($object == NULL) {
			...
			$this->load->model('core_fbutils');
			$result = $this->core_fbutils->get_facebook_object($object, $this->session->userdata('facebook_uid'), $this->session->userdata('access_token'));
	...
	function friends() {
		$this->load->model('core_fbutils');
		$result = $this->core_fbutils->get_facebook_object('friends', $this->session->userdata('facebook_uid'), $this->session->userdata('access_token'));
	...
	function likes() {
		$this->load->model('core_fbutils');
		$result = $this->core_fbutils->get_facebook_object('likes', $this->session->userdata('facebook_uid'), $this->session->userdata('access_token'));
