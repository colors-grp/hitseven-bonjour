...
class Account_model extends CI_Model {	
	 ...
	 */
	function create($username, $email = NULL, $birthday, $password = NULL)
	{
		if ($birthday !== NULL)
		{
		...
		$this->load->helper('date');
		$this->db->insert('a3m_account', array('username' => $username, 'email' => $email, 'dateofbirth' => $birthday, 'password' => isset($hashed_password) ? $hashed_password : NULL, 'createdon' => mdate('%Y-%m-%d %H:%i:%s', now())));
