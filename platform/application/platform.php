...
class Platform extends CI_Controller {
	...
	function get_dashboard_info($user_id, $interset_cats) {
		...
		foreach ($interset_cats->result() as $int_cat) {
                    ...
                    $cur_rank = $this->scoreboard_model->get_user_rank($user_id, $int_cat->id)->row(0)->rank;
                    $total_scores = $this->scoreboard_model->get_total_scores($int_cat->id)->row(0)->total;
                    $difference = 1;
                    if($cur_rank == 1){
                        $difference = 0;
                    }
                    ...
                    $result[$i]['users'] = $this->scoreboard_model->get_dash_ranks($user_id, $int_cat->id - 1, $difference, $viewLimit);
